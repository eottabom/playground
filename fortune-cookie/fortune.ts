...
class FortuneCookie {
  ...
  private async initialize(): Promise<void> {
    ...
    if (!data) {
      try {
        ...
        this.applyI18nData(await response.json() as I18nFileData);
      } catch (error) {
  ...
  private loadI18nData(): boolean {
    const embedded = document.getElementById("fortuneI18nData");
    if (!embedded?.textContent) return false;
    try {
      this.applyI18nData(JSON.parse(embedded.textContent) as I18nFileData);
      return true;
    } catch (error) {
  ...
  private applyI18nData(data: I18nFileData): void {
    this.content = {
      ko: data.ko.fortune,
      en: data.en.fortune,
    };
