interface I18nStrings {
  ...
  keywordLabel: string;
  colorLabel: string;
  introLabelOne: string;
  introValueOne: string;
  introLabelTwo: string;
  introValueTwo: string;
  sceneAria: string;
}
...
interface FortuneLocaleData extends I18nStrings {
  tags: string[];
...
function drawCookieHalf(canvas: HTMLCanvasElement, side: "left" | "right"): void {
  ...
  for (let i = 0; i < 10; i++) {
    ...
    ctx.ellipse(
      bx,
      by,
      w * (0.006 + random() * 0.008),
      h * (0.004 + random() * 0.008),
      random() * Math.PI,
      0,
      Math.PI * 2
    );
...
class FortuneCookie {
  ...
  private miniSlip: HTMLElement;
  private miniSlipText: HTMLElement;
  private miniSlipMeta: HTMLElement;
  ...
  private introLabelOne: HTMLElement;
  private introValueOne: HTMLElement;
  private introLabelTwo: HTMLElement;
  private introValueTwo: HTMLElement;
  ...
  private lang: string;
  private currentFortuneIdx = -1;
  private currentLucky = 0;
  private currentTagIdx = 0;
  ...
  private async initialize(): Promise<void> {
    ...
    if (embedded?.textContent) {
      try {
        const data = JSON.parse(embedded.textContent) as I18nFileData;
        this.content = {
          ko: data.ko.fortune,
          en: data.en.fortune,
        };
    ...
    try {
      ...
      const data = await response.json() as I18nFileData;
      this.content = {
        ko: data.ko.fortune,
        en: data.en.fortune,
      };
  ...
  private crack(): void {
    if (this.isCracked || !this.isReady) return;
    this.isCracked = true;
